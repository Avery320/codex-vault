 ...
 */
export type CometMcpServerMode = 'read' | 'read-write';

export interface CometMcpServerOptions {
  /** Supplies the workspace used by each tool invocation. */
  workspaceProvider: CometMcpWorkspaceProvider;
  /** Access mode. See {@link CometMcpServerMode}. */
  mode: CometMcpServerMode;
  /** Optional multi-vault operations exposed by COMET. */
...
const READ_ONLY_INSTRUCTIONS =
  'This Comet MCP server is running in read-only mode. ' +
...
/**
 * MCP server that exposes a Comet workspace's knowledge graph and content
 * to AI agents. Construct, then `connect(transport)` to start listening.
 *
 * The Comet instance is injected — this library does not pick a filesystem
 ...
 */
export class CometMcpServer {
  private readonly mcp: McpServer;
  private readonly workspaceProvider: CometMcpWorkspaceProvider;

  constructor(opts: CometMcpServerOptions) {
