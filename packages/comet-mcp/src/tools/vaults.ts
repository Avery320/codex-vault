import { CometError } from '@comet/core';
import { z } from 'zod';
...
export function registerVaultTools(
  ...
  register(
    ...
    async args => {
      ...
      if (!active) {
        throw new CometError('resource_not_found', 'Vault could not be opened.');
      }
      return json({
        active_vault: { ...active.vault, active: true },
        vaults: await vaultManager.listVaults(),
  ...
  if (pickVaultFolder) {
    register(
      ...
      {
        description:
          'Open the operating system folder picker for the COMET UI.',
        inputSchema: {},
        annotations: {
          readOnlyHint: true,
