...
export function registerResourceTools(
  ...
  register(
    ...
    async args => {
      ...
      const items = listNotes(comet.workspace, {
        type: args.type,
  ...
  register(
    ...
    async args => {
      ...
      const resource = resolveNote(comet.workspace, ref);
      const detail = noteShowData(comet.workspace, comet.graph, resource);
    ...
    async args => {
      ...
      if (args.path) {
        ...
        if (!uri.path.toLocaleLowerCase().endsWith('.md')) {
          throw new CometError('invalid_input', '`path` must end with `.md`.');
        }
      }

      const result = await noteCreate(comet, {
        title: args.title,
        dir: args.dir,
        uri,
      ...
      const resource = comet.workspace.find(result.uri);
      return json({
  ...
  register(
    ...
    async args => {
      const { comet, dataStore, rootUri } = requireWorkspace(workspaceProvider);
      const uri = parseUriInput(args.uri, rootUri);
      if (args.content === undefined && args.properties === undefined) {
        throw new CometError(
          'invalid_input',
      ...
      const nextContent = buildNextContent(currentContent, args);
      await writeWorkspaceResource(comet, uri, nextContent);
      return json({
        uri: uriToOutputString(uri, rootUri),
  ...
  register(
    ...
    async args => {
      ...
      await noteDelete(comet, resource);
      return json({
        deleted: true,
        trashed: true,
      });
  ...
  register(
    ...
    async args => {
      ...
      const resource = resolveNote(comet.workspace, { uri: oldUri });
      const result = await noteMove(comet, resource, newUri);
      return json({
