...
const CLI_HELP = `Usage: comet mcp [options]

COMET uses this package only as its Node.js stdio MCP runtime.

Commands:
  mcp         Run the COMET MCP server
...
async function dispatch(
  ...
  try {
    ...
    if (process.env.COMET_DEBUG && stack) logger.error(stack);
    return 1;
  }
}
