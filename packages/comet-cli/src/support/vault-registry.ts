...
function parseRegistry(value: unknown): VaultRegistryData {
  ...
  const vaults = candidate.vaults.map(vault => {
    if (
      ...
      throw new Error('Invalid vault entry in COMET registry.');
    }
    return { ...vault };
  });
