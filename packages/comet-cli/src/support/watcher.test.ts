...
import { URI } from '@comet/core';
import { VaultFilePolicy } from '@comet/mcp';
...
describe('NodeWatcher', () => {
  it('fires onDidCreate when a file is added after the watcher starts', async () => {
    const dir = mkdtempSync(path.join(tmpdir(), 'comet-watcher-create-'));
  ...
  it('fires onDidDelete when a file is removed', async () => {
    const dir = mkdtempSync(path.join(tmpdir(), 'comet-watcher-delete-'));
  ...
  it('debounces rapid change events', async () => {
    const dir = mkdtempSync(path.join(tmpdir(), 'comet-watcher-debounce-'));
  ...
  it('does not fire onDidCreate for pre-existing files (ignoreInitial)', async () => {
    const dir = mkdtempSync(path.join(tmpdir(), 'comet-watcher-existing-'));
  ...
  it('uses the vault policy to ignore excluded directories', async () => {
    const dir = mkdtempSync(path.join(tmpdir(), 'comet-watcher-policy-'));
