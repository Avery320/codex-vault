...
export function createMarkdownParser(): ResourceParser {
  ...
  const cometParser: ResourceParser = {
      ...
      visit(tree, node => {
        ...
        if (node.type === 'definition') {
          ...
          localDefinitions.push({
            ...
            range: astPositionToCometRange(node.position!),
          });
        }
  ...
  visit: (node, note) => {
    if (node.type === 'text') {
      ...
      for (const tag of tags) {
        const start = astPointToCometPosition(node.position!.start);
...
const sectionsPlugin: ParserPlugin = {
  ...
  visit: (node, note) => {
    if (node.type === 'heading') {
      ...
      const start = astPositionToCometRange(node.position!).start;
  ...
  onDidVisitTree: (tree, note) => {
    const end = Position.create(
      astPointToCometPosition(tree.position.end).line + 1,
      0
    );
  ...
  visit: (node, note, noteSource) => {
    if (node.type === 'wikiLink') {
      ...
      const range = isEmbed
        ...
        : astPositionToCometRange(node.position!);
 ...
 * @returns Comet Position  (0-indexed)
 */
const astPointToCometPosition = (point: Point): Position => {
 ...
 * @returns Comet Range  (0-indexed)
 */
const astPositionToCometRange = (pos: AstPosition): Range =>
  Range.create(
    pos.start.line - 1,
    pos.start.column - 1,
    pos.end.line - 1,
