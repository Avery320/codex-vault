...
describe('Identifier computation', () => {
  it('should compute the minimum identifier to resolve a name clash', () => {
    ...
    const ws = new CometWorkspace([], '.md').set(first).set(second).set(third);
  ...
  it('should handle case-sensitive filenames correctly (#1303)', () => {
    const workspace = new CometWorkspace([], '.md');
...
describe('resolveUri', () => {
  ...
  it.each([
    ...
    expect(new CometWorkspace(roots).resolveUri(input, relativeTo).path).toBe(
      expected
    );
  });
  ...
  it.each([
    ...
    (scheme, authority) => {
      ...
      const result = new CometWorkspace([]).resolveUri(
  ...
  describe('Windows drive paths', () => {
    it.each([
      ...
      expect(new CometWorkspace([winRoot]).resolveUri(input).path).toBe(
...
describe('find with workspace-relative absolute paths', () => {
  it('should find a resource stored at a real absolute path via a workspace-relative path', () => {
    const root = URI.file('/workspace');
    const ws = new CometWorkspace([root]);
