...
export class CometWorkspace implements IDisposable {
  ...
  private _directoryIndexPriority(uri: URI): number {
    ...
    return CometWorkspace.DIRECTORY_INDEX_NAMES.indexOf(name);
  }
  ...
  public getIdentifier(forResource: URI, exclude?: URI[]): string {
    ...
    let identifier = CometWorkspace.getShortestIdentifier(
      forResource.path,
  ...
  public getDirectoryIdentifier(forResource: URI): string | null {
    ...
    return CometWorkspace.getShortestIdentifier(dirPath, otherDirPaths);
  }
  ...
  public find(reference: URI | string, baseUri?: URI): Resource | null {
    ...
    if (CometWorkspace.isIdentifier(path)) {
  ...
  ): Promise<CometWorkspace> {
    const workspace = new CometWorkspace(roots, defaultExtension);
