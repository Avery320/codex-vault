...
function snapshot(graph: CometGraph): GraphSnapshot {
...
function expectEquivalentToRebuild(
  incremental: CometGraph,
  workspace: CometWorkspace
) {
  const scratch = CometGraph.fromWorkspace(workspace);
  try {
...
describe('Incremental graph update equivalence', () => {
  it('matches a rebuild after updating a note to add and remove links', () => {
    ...
    const graph = CometGraph.fromWorkspace(workspace, true);
  ...
  it('matches a rebuild after a rename (delete old + add new), preserving outgoing links', () => {
    // Comet has no rename event: a rename arrives as delete(old) + add(new).
